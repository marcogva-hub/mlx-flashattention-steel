...
namespace metal
{
  ...
  struct simdgroup_event {
    ...
    METAL_FUNC void async_copy(
      ...
      const ushort rows = transpose_matrix ? src_tile_dimensions.x : src_tile_dimensions.y;
      const ushort cols = transpose_matrix ? src_tile_dimensions.y : src_tile_dimensions.x;
      for (ushort r = 0; r < rows; ++r) {
        for (ushort c = 0; c < cols; ++c) {
          const ushort src_r = transpose_matrix ? c : r;
          const ushort src_c = transpose_matrix ? r : c;
          if (r < dst_tile_dimensions.y && c < dst_tile_dimensions.x &&
              src_r < src_tile_dimensions.y && src_c < src_tile_dimensions.x) {
            dst[(ushort)(r * dst_elements_per_row + c)] =
                src[(uint)(src_r * src_elements_per_row + src_c)];
          }
        }
      }
    }
    ...
    METAL_FUNC void async_copy(
      ...
      for (ushort r = 0; r < rows; ++r) {
        for (ushort c = 0; c < cols; ++c) {
          ...
          if (r < dst_tile_dimensions.y && c < dst_tile_dimensions.x &&
              src_r < src_tile_dimensions.y && src_c < src_tile_dimensions.x) {
            dst[(uint)(r * dst_elements_per_row + c)] =
                src[(ushort)(src_r * src_elements_per_row + src_c)];
          }
        }
      }
    }
