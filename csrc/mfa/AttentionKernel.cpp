...
// Construct kernel: generate shader source only (no Metal device required).
AttentionKernel::AttentionKernel(AttentionKernelDescriptor descriptor) {
  ...
  disableAsyncCopy = true; // macOS 26+: __asm air.simdgroup_async_copy intrinsics removed
...
std::string AttentionKernel::createSource() const noexcept {
  ...
  // mlx-mfa: use static threadgroup array instead of [[threadgroup(0)]] arg,
  // which would require setThreadgroupMemoryLength (private MLX API).
  source.SetValue("TGMEM_SIZE", std::to_string(threadgroupMemoryAllocation));
  source += R"(
      uint3 gid [[threadgroup_position_in_grid]],
      ushort sidx [[simdgroup_index_in_threadgroup]],
      ushort lane_id [[thread_index_in_simdgroup]]
    ) {
      threadgroup uchar threadgroup_block[{{TGMEM_SIZE}}];
...
std::string AttentionKernel::createConstants() const noexcept {
  // mlx-mfa: MFAParams is passed at buffer(10) (not buffer(5), which is occupied
  // by the D operand in backward kernels).  A unified fixed-layout struct is used
  // for ALL kernel types so the C++ MFAParams struct matches bit-for-bit.
  // Only operands relevant to each kernel type get #define aliases; the others
  // remain in the Metal struct but are never read, so their C++ values are ignored.

  // Operands that need #define aliases for this kernel type.
  std::vector<AttentionOperand> relevant_operands;
  switch (type.value) {
  case AttentionKernelType::forward:
    relevant_operands = {AttentionOperand::Q, AttentionOperand::K, AttentionOperand::V, AttentionOperand::O};
    break;
  case AttentionKernelType::backwardQuery:
    relevant_operands = {AttentionOperand::Q, AttentionOperand::K, AttentionOperand::V, AttentionOperand::O, AttentionOperand::dO, AttentionOperand::dQ};
    break;
  case AttentionKernelType::backwardKeyValue:
    relevant_operands = {AttentionOperand::Q, AttentionOperand::K, AttentionOperand::V, AttentionOperand::O, AttentionOperand::dO, AttentionOperand::dV, AttentionOperand::dK};
    break;
  }

  // Build #define aliases (and optional leading-dimension aliases) only for
  // the operands active in this kernel type.
  std::string defines;
  for (const auto& operand : relevant_operands) {
    defines += "#define " + operand.name() + "_batch_stride (params." + operand.name() + "_batch_stride)\n";
    if (leadingDimensions[operand].value_or(false)) {
      defines += "#define " + operand.name() + "_leading_dimension (params." + operand.name() + "_leading_dimension)\n";
    }
  }

  // Unified fixed Metal struct — same layout for forward AND backward kernels.
  // Fields dO/dQ/dK/dV are zero for forward; Q/K/V/O are also present for backward.
  // L and D do not need batch-stride fields: their pointer offsets are computed
  // via the inline formula "(gid.z * Hq + gid.y) * R" in adjustOffsets.
  return
    "struct MFAParams {\n"
    "  uint R;\n"
    "  uint C;\n"
    ...
    "};\n"
    // No program-scope binding: params is a kernel parameter (createBufferBindings),
    // so all #define macros below resolve correctly inside the kernel body.
    "#define R (params.R)\n"
    "#define C (params.C)\n"
    "#define Hq (params.Hq)\n"
    ...
    // Backward kernels need the raw attention scale (not multiplied by log2(e)).
    // dot_product_scale = scale * log2(e), so:
    // dot_product_scale_derivative = scale = dot_product_scale * ln(2)
    + std::string(type.value != AttentionKernelType::forward
...
std::string AttentionKernel::createBufferBindings() const noexcept {
  ...
  // mlx-mfa: params struct replaces Metal function constants (see createConstants).
  // buffer(10) avoids conflicts with operand buffers (highest occupied = dQ at 9).
  output += "  constant MFAParams& params [[buffer(10)]],\n";
  return output;
}
...
std::string AttentionKernel::operandLocationWithHeadOffsetValue(AttentionOperand operand) const noexcept {
  ...
  if (operand.value == AttentionOperand::L || operand.value == AttentionOperand::D) {
    ...
    // mlx-mfa: MLX uses BHND layout, so head stride = headDim * seqLen regardless
    // of transposeState (which only controls GEMM inner-loop access pattern).
...
std::string AttentionKernel::loopForward() const noexcept {
  ...
  source.SetValue("CAUSAL_MASK", causalMask());
...
std::string AttentionKernel::loopBackwardKeyValue() const noexcept {
  ...
  source.SetValue("CAUSAL_MASK_TRANSPOSED", causalMaskTransposed());
  source.SetValue("MASK_ATTENTION_MATRIX_EDGE", maskAttentionMatrixEdge());
  source.SetValue("SOFTMAX", softmax(false));
...
std::string AttentionKernel::causalMask() const noexcept {
  // Inject causal masking after Q*K^T: set S[q_row, k_col] = -large when
  // k_col > q_row (future positions must not be attended to).
  //
  // S_sram[j] holds an 8x8 simdgroup matrix covering K-columns
  //   [c + j*8, c + j*8 + 8).
  // Per-thread layout (from morton_order):
  //   Q-row of this thread  = parallelization_group_offset + sidx*8 + morton_offset.y
  //   K-col of element [0]  = c + j*8 + morton_offset.x
  //   K-col of element [1]  = c + j*8 + morton_offset.x + 1
  float logBase2E = 1.442695041;
  CodeWriter source;
  source.SetValue("BLOCK_DIMENSIONS_TRAVERSAL", std::to_string(blockDimensions[1]));
  source.SetValue("LOG_BASE_2E", std::to_string(logBase2E));
  ...
  // Causal mask: future key positions (k_col > q_row) are set to -large.
  if (causal) {
    uint q_row = parallelization_group_offset + uint(sidx) * 8 + uint(morton_offset.y);
    const {{REGISTER_NAME_S}} mask_value =
      (0.875 / {{LOG_BASE_2E}}) * -numeric_limits<{{REGISTER_NAME_S}}>::max();
    #pragma clang loop unroll(full)
    for (ushort j = 0; j < {{BLOCK_DIMENSIONS_TRAVERSAL}} / 8; j++) {
      uint k_col_base = c + uint(j) * 8 + uint(morton_offset.x);
      auto S_elements = S_sram[j].thread_elements(); // Metal: auto* requires explicit AS; auto deduces thread T*
      if (k_col_base     > q_row) { (*S_elements)[0] = mask_value; }
      if (k_col_base + 1 > q_row) { (*S_elements)[1] = mask_value; }
    }
  }
...
std::string AttentionKernel::causalMaskTransposed() const noexcept {
  // Inject causal masking for the backwardKeyValue kernel after K*Q^T.
  //
  // In backwardKeyValue, S^T[k_col, q_row] = K[k_col] · Q[q_row].
  // This mirrors P[q_row, k_col] from the forward pass.
  // Causal masking: P[q, k] = 0 if k > q, so S^T[k, q] = -large if k > q.
  //
  // Per-thread layout for S^T (parallelization = K cols, traversal = Q rows):
  //   K-col (row of S^T) = parallelization_group_offset + sidx*8 + morton_offset.y
  //   Q-row base (col of S^T, element [0]) = r + j*8 + morton_offset.x
  //   Q-row base (col of S^T, element [1]) = r + j*8 + morton_offset.x + 1
  float logBase2E = 1.442695041;
  CodeWriter source;
  source.SetValue("BLOCK_DIMENSIONS_TRAVERSAL", std::to_string(blockDimensions[1]));
  source.SetValue("LOG_BASE_2E", std::to_string(logBase2E));
  ...
  // Transposed causal mask: query positions where q_row < k_col are masked.
  if (causal) {
    uint k_col = parallelization_group_offset + uint(sidx) * 8 + uint(morton_offset.y);
    const {{REGISTER_NAME_S}} mask_value =
      (0.875 / {{LOG_BASE_2E}}) * -numeric_limits<{{REGISTER_NAME_S}}>::max();
    #pragma clang loop unroll(full)
    for (ushort j = 0; j < {{BLOCK_DIMENSIONS_TRAVERSAL}} / 8; j++) {
      uint q_row_base = r + uint(j) * 8 + uint(morton_offset.x);
      auto S_elements = S_sram[j].thread_elements();
      if (k_col > q_row_base    ) { (*S_elements)[0] = mask_value; }
      if (k_col > q_row_base + 1) { (*S_elements)[1] = mask_value; }
    }
  }
