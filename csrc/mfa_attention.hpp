...
class MFAGNAForward : public mlx::core::Primitive {
 public:
  ...
  bool is_equivalent(const mlx::core::Primitive& other) const override {
    ...
    if (!o) return false;
    if (params_.head_dim != o->params_.head_dim ||
        params_.scale    != o->params_.scale    ||
        params_.gqa_factor != o->params_.gqa_factor ||
        params_.ndim     != o->params_.ndim) return false;
    for (int d = 0; d < params_.ndim; d++) {
      if (params_.seq_shape[d]   != o->params_.seq_shape[d]   ||
          params_.window_size[d] != o->params_.window_size[d] ||
          params_.stride[d]      != o->params_.stride[d]) return false;
    }
    return true;
  }
