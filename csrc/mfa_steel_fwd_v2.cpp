...
struct MFASteelParams {
  ...
  // K-tile so the direct device read stays in bounds.  The V sibling already
  // clamps; the K over-read is masked to -INF by the K-boundary mask BEFORE
  // P@V, so the in-bounds result is byteΔ-identical — this only removes the UB
  // OOB read (a wrong-but-discarded score either way).
  ss << "      short k_row = sn;\n";
  ...
  ss << "          // key. The direct device read has no bounds check (unlike the\n";
  ss << "          // !DIRECT load_safe zero-pad), so an OOB key (past kL; for the\n";
  ss << "          // last head, past the V buffer) would read NaN/stale-pool data.\n";
  ss << "          // P is already masked to 0 for these keys (K-boundary mask),\n";
  ss << "          // but 0*NaN = NaN would corrupt O — so read a valid (finite) V\n";
  ss << "          // instead; 0*finite = 0 is the correct masked contribution.\n";
