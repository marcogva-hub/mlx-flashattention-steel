"""V6NAX backward integration via flash_attention() VJP (Phase 2 Section E).

SHIP_OPT_IN posture (per perf regression discovered Phase 3):
- Default behaviour: V6NAX backward NOT engaged; flash_attention() VJP
  uses STEEL backward / SDPA-vjp per existing dispatch policy.
- Opt-in: set MFA_ENABLE_V6_BACKWARD=1 to engage V6NAX backward kernels.

Tests verify:
- Opt-in correctness: V6NAX backward produces correct gradients vs SDPA-vjp.
- Opt-in path engaged: V6NAX-on vs V6NAX-off produce numerically different output.
...
@pytest.fixture
def enable_v6nax_bwd(monkeypatch):
...
# ---------------------------------------------------------------------------
# Opt-in correctness (MFA_ENABLE_V6_BACKWARD=1)
# ---------------------------------------------------------------------------
def test_v6nax_bwd_optin_d128_fp16_qL1024(enable_v6nax_bwd):
...
def test_v6nax_bwd_optin_d128_fp16_qL512(enable_v6nax_bwd):
...
def test_v6nax_bwd_optin_bf16(enable_v6nax_bwd):
...
# ---------------------------------------------------------------------------
def test_v6nax_bwd_optin_engages_path(enable_v6nax_bwd, monkeypatch):
    """V6NAX-enabled output should differ from V6NAX-disabled (FP16
    rounding between V6NAX NAX kernels and SDPA-vjp paths)."""
    q, k, v = _make(1, 4, 4, 1024, 1024, 128, 45, mx.float16)
    dQ_on, _, _ = _grads(q, k, v)
    monkeypatch.delenv("MFA_ENABLE_V6_BACKWARD", raising=False)
    dQ_off, _, _ = _grads(q, k, v)
    diff = _rmse(dQ_on, dQ_off)
    assert diff > 0, "V6NAX backward did not engage (V6NAX-on == V6NAX-off)"


# ---------------------------------------------------------------------------
# Default-off behaviour
# ---------------------------------------------------------------------------
def test_v6nax_bwd_default_off_falls_back():
    """Without MFA_ENABLE_V6_BACKWARD=1, fallback path is engaged.
...
def test_v6nax_bwd_optin_d64_small_nk_engages_v6nax(enable_v6nax_bwd):
    """v2.37.0+: DC12 routing-parity constraint RELAXED.  V6NAX backward
    now engages for D=64 small-Nk shapes (v6_nax_forward called with
    force_v6nax=True so V6NAX forward path emits natural-log lse).

    Pre-v2.37.0 this case fell through to SDPA-vjp; now V6NAX NAX kernels
    handle it.  Verify (a) correctness vs SDPA-vjp within FP16/FP32
    floor, (b) V6NAX path is engaged (output differs from SDPA-vjp by
    FP16 rounding)."""
    ...
    assert _rmse(dK, dK_ref) < 1e-3
    assert _rmse(dV, dV_ref) < 1e-2
    # (b) V6NAX path engaged (non-zero diff vs identical SDPA-vjp fallback)
    assert _rmse(dQ, dQ_ref) > 0.0, (
        "V6NAX backward did not engage on D=64 small-Nk")


# ---------------------------------------------------------------------------
# v2.37.2 regression: backend="auto" carve-out actually engages V6NAX backward
# ---------------------------------------------------------------------------
...
def test_v6nax_bwd_v2372_carveout_engages_on_d64_qL4096(enable_v6nax_bwd):
    ...
    SDPA-vjp in v2.37.0/v2.37.1 — defeating the documented MFA_ENABLE_V6_BACKWARD
    contract.

    The v2.37.2 carve-out forces use_mfa=True when env=1 and shape
    qualifies (D=64, qL ≥ 4096, non-causal, f16/bf16, NAX) so the
    custom-vjp _impl is constructed and V6NAX backward actually engages.

    Regression check: V6NAX backward output must differ from SDPA-vjp by
    a non-zero FP16-rounding amount (proves V6NAX path engaged, not SDPA
    fallback)."""
    ...
    assert _rmse(dK, dK_ref) < 1e-3
    assert _rmse(dV, dV_ref) < 1e-2
    # Engagement: V6NAX path must produce different bits than SDPA fallback
    assert _rmse(dQ, dQ_ref) > 0.0, (
        "v2.37.2 carve-out failed: V6NAX backward did NOT engage via "
        "backend='auto' at D=64 qL=4096 — silent SDPA fallback")


def test_v6nax_bwd_v2372_carveout_does_not_engage_below_threshold(enable_v6nax_bwd):
    """v2.37.2 carve-out is shape-gated: qL < 4096 should NOT engage V6NAX
    (V6NAX loses end-to-end at small Nk vs SDPA-vjp).  The carve-out only
...
def test_v6nax_bwd_v2372_carveout_does_not_engage_d128_below_floor(enable_v6nax_bwd):
...
def test_v6nax_bwd_carveout_engages_d128_above_floor(enable_v6nax_bwd):
    """v2.50 Prompt 5b Section D: D=128 + qL>=2048 + fp16 NOW engages
    V6NAX backward split kernels via AUTO.  Gradients differ from SDPA-vjp
    by FP16 rounding (non-zero RMSE) — engagement signature."""
    q, k, v = _make(1, 4, 4, 4096, 4096, 128, 55, mx.float16)
    scale = 1.0 / math.sqrt(128)
    dQ, dK, dV = _grads_auto(q, k, v)
    dQ_ref, dK_ref, dV_ref = _sdpa_grads(q, k, v, scale)
    # V6NAX split engaged → gradients NOT bit-identical (FP16 rounding diff)
    diffs = [_rmse(dQ, dQ_ref), _rmse(dK, dK_ref), _rmse(dV, dV_ref)]
    assert max(diffs) > 1e-9, (
        f"D=128 qL=4096 V6NAX backward should engage via AUTO; "
        f"max diff vs SDPA = {max(diffs):.2e}"
    )


def test_v6nax_bwd_v2372_carveout_default_on_ii12(monkeypatch):
    ...
    V6NAX (V6NAX-floor rmse vs SDPA-vjp); the opt-out restores bit-identical
    SDPA-vjp."""
...
# ---------------------------------------------------------------------------
# v2.38.x regression: outer flash_attention guards prevent V6NAX carve-out
...
# placeholder + _v6nax_backward_carveout flash_attention-level concern).
# ---------------------------------------------------------------------------
def test_v38x_carveout_does_not_engage_with_softcap_nonzero(enable_v6nax_bwd):
    """v2.38.x: even with env=1 + qualifying shape, softcap≠0 must NOT
    engage the V6NAX carve-out.  The outer guards in flash_attention()
    delegation block exclude this combination by design.  Without these
    guards (Phase A consolidation bug), V6NAX would silently engage and
    ...
    # as "V6NAX carve-out engaged silently routing through plain MFA".
    ...
    # but the V6NAX backward would produce VASTLY different gradients
    ...
    for ref, fa in zip(g_ref, g_fa):
        ...
        # FP16 noise floor.  V6NAX carve-out engagement would produce
        ...
        assert rel < 0.5, (
            ...
            f"the V6NAX carve-out silently engaged despite softcap≠0 — "
