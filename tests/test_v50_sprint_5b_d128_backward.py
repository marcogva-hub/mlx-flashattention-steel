"""v2.50 Prompt 5b Section D — D=128 V6NAX backward broadening tests.

Per Marco's Option A decision: lift `_v6nax_backward_carveout` D=128 hard-
...
- Axe 2 (PUBLIC API): `mx.grad(flash_attention(q, k, v))` with D=128 +
  qL>=2048 + fp16/bf16 + `MFA_ENABLE_V6_BACKWARD=1` engages V6NAX backward
...
_skipif_no_nax = pytest.mark.skipif(
    not _HAS_NAX, reason="V6NAX backward D=128 broadening requires M5+ NAX hardware"
)
...
# ---------------------------------------------------------------------------
# Axe 1 + Axe 2: D=128 V6NAX backward output matches SDPA-vjp baseline
# ---------------------------------------------------------------------------
class TestD128BackwardCorrectness:
    """V6NAX backward D=128 output matches `mx.vjp(SDPA)` baseline within
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d128_non_causal_qL2048_matches_sdpa(self, monkeypatch):
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d128_non_causal_qL4096_matches_sdpa(self, monkeypatch):
        monkeypatch.setenv("MFA_ENABLE_V6_BACKWARD", "1")
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d128_non_causal_qL8192_matches_sdpa(self, monkeypatch):
        monkeypatch.setenv("MFA_ENABLE_V6_BACKWARD", "1")
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d128_causal_qL2048_matches_sdpa(self, monkeypatch):
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d128_causal_qL4096_matches_sdpa(self, monkeypatch):
        monkeypatch.setenv("MFA_ENABLE_V6_BACKWARD", "1")
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d128_bf16_matches_sdpa(self, monkeypatch):
        monkeypatch.setenv("MFA_ENABLE_V6_BACKWARD", "1")
...
# ---------------------------------------------------------------------------
# Axe 2: PUBLIC API engagement — V6NAX split path must engage for D=128
# ---------------------------------------------------------------------------
class TestD128BackwardPublicAPIEngagement:
    """Verify the PUBLIC API path (`mx.grad(flash_attention(...))`) engages
    V6NAX backward split kernels for D=128.  Engagement detection is
    differential: V6NAX split gradients differ from SDPA-vjp gradients by
    FP16 rounding (non-zero RMSE).  SDPA fallback produces bit-identical
    gradients to the explicit SDPA reference path."""

    @_skipif_no_nax
    def test_v6nax_bwd_d128_routing_public_api_engages_v6nax_split(self, monkeypatch):
        """When env=1 + D=128 + qL>=2048 + fp16, AUTO must engage V6NAX split
        ...
        dQ_r, dK_r, dV_r = _grad_sdpa(q, k, v, dO, causal=False)
        _AE(dQ_a, dK_a, dV_a, dQ_r, dK_r, dV_r); mx.synchronize()
        # V6NAX engaged → gradients are NOT bit-identical to SDPA-vjp (non-zero diff)
        diffs = [_rmse(dQ_a, dQ_r), _rmse(dK_a, dK_r), _rmse(dV_a, dV_r)]
        max_diff = max(diffs)
        assert max_diff > 1e-7, (
            f"V6NAX backward D=128 did NOT engage (max diff vs SDPA = {max_diff:.2e}; "
            f"expected non-zero FP16 rounding diff if V6NAX split kernel ran)"
        )
...
class TestD128BackwardEdges:
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d128_below_qL_threshold_falls_back_sdpa(self, monkeypatch):
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d128_fp32_falls_back_sdpa(self, monkeypatch):
        """D=128 fp32 MUST fall back to SDPA-vjp (V6NAX backward is fp16/bf16
        only)."""
        monkeypatch.setenv("MFA_ENABLE_V6_BACKWARD", "1")
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d64_unchanged_post_d128_broadening(self, monkeypatch):
        """Regression check: D=64 qL=4096 V6NAX backward (pre-Section-D
        ...
        dQ_r, dK_r, dV_r = _grad_sdpa(q, k, v, dO, causal=False)
        _AE(dQ_a, dK_a, dV_a, dQ_r, dK_r, dV_r); mx.synchronize()
        # D=64 engages V6NAX fused kernel — differs from SDPA-vjp by FP16 rounding
        diffs = [_rmse(dQ_a, dQ_r), _rmse(dK_a, dK_r), _rmse(dV_a, dV_r)]
        max_diff = max(diffs)
        assert max_diff > 1e-7, (
            f"D=64 qL=4096 V6NAX backward should still engage post-broadening; "
    ...
    @_skipif_no_nax
    def test_v6nax_bwd_d128_env_unset_falls_back(self, monkeypatch):
        """Without env=1, D=128 must NOT engage V6NAX (bit-identical to SDPA)."""
        ...
        for (a, r, name) in [(dQ_a, dQ_r, "dQ"), (dK_a, dK_r, "dK"), (dV_a, dV_r, "dV")]:
            assert _rmse(a, r) < 1e-7, (
                f"D=128 env unset {name} should NOT engage V6NAX; "
...
# ---------------------------------------------------------------------------
class TestV6NAXBackwardCarveoutD128:
    """Direct test of `_v6nax_backward_carveout` post-broadening."""
...
def test_adversarial_magnitude_finite(monkeypatch):
    ...
    D=128 V6NAX backward gradients finite (fp16-overflow guard)."""
    monkeypatch.setenv("MFA_ENABLE_V6_BACKWARD", "1")
