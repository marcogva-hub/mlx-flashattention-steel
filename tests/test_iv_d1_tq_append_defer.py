...
def test_default_tqv_true_keeps_eager_eval():
    """Default tq_v=True must NOT defer (packed-V pools are written-but-unread)."""
    ctx = TurboQuantPagedInferenceContext(num_blocks=32, block_size=bs, H_kv=Hkv,
                                          D=D, tq_bits=bits, tq_v=True)
    mx.random.seed(1)
    pq = (mx.random.uniform(-1, 1, (1, Hq, 128, D)) * 0.1).astype(mx.float16)
    pk = (mx.random.uniform(-1, 1, (1, Hkv, 128, D)) * 0.1).astype(mx.float16)
    pv = (mx.random.uniform(-1, 1, (1, Hkv, 128, D)) * 0.1).astype(mx.float16)
    ctx.prefill(pq, pk, pv)
    sq, sk, sv = _step_inputs(0)
    o = ctx.step(sq, sk, sv, scale=1.0 / math.sqrt(D))
    mx.eval(o)
    assert bool(mx.isfinite(o).all().item())
