"""V34 routing helpers (`_v34_eligible`, `_v34_backward_vjp`) — direct tests.

Per Sprint v2.38.0 DP2-HIGH-01 compound finding (audit M4-MEDIUM-01).
The helpers were extracted from `_make_mfa_custom` to:
- Deduplicate the V34-eligibility predicate (was triplicated)
- Make the 3-kernel V34 backward dispatch testable in isolation
...
`test_flash_attention_v34_backward.py` and `test_release_notes_perf_claims.py`
...
- Axis 2 (path entered via PUBLIC API): covered in
  test_flash_attention_v34_backward.py — these helper tests are direct
...
from mlx_mfa.attention import _v34_eligible, _v34_backward_vjp
...
# ---------------------------------------------------------------------------
# _v34_eligible() — predicate truth table
# ---------------------------------------------------------------------------
class TestV34Eligible:
    ...
    def test_d128_fp16_noncausal_env_set_returns_true(self, monkeypatch):
        ...
        The flash_attention()-level carve-out (`_v34_backward_carveout`
    ...
    def test_d64_fp32_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("MFA_ENABLE_V34_BACKWARD", "1")
        assert _v34_eligible(64, mx.float32, causal=False) is False

    def test_d256_returns_false(self, monkeypatch):
        """head_dim ∉ {64, 128} excluded."""
        monkeypatch.setenv("MFA_ENABLE_V34_BACKWARD", "1")
        assert _v34_eligible(256, mx.float16, causal=False) is False

    def test_d32_returns_false(self, monkeypatch):
        """head_dim ∉ {64, 128} excluded (small D)."""
        monkeypatch.setenv("MFA_ENABLE_V34_BACKWARD", "1")
        assert _v34_eligible(32, mx.float16, causal=False) is False
    ...
    def test_env_unset_returns_false(self):
        ...
        assert _v34_eligible(64, mx.float16, causal=False) is False
    ...
    def test_env_set_to_zero_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("MFA_ENABLE_V34_BACKWARD", "0")
        assert _v34_eligible(64, mx.float16, causal=False) is False

    def test_env_set_to_yes_returns_false(self, monkeypatch):
        monkeypatch.setenv("MFA_ENABLE_V34_BACKWARD", "yes")
        assert _v34_eligible(64, mx.float16, causal=False) is False


# ---------------------------------------------------------------------------
# _v34_backward_vjp() — output shapes + dispatch routing
# ---------------------------------------------------------------------------
...
class TestV34BackwardVjp:
    """Direct invocation of `_v34_backward_vjp` with synthetic inputs.

    Caller (`_make_mfa_custom._backward`) is responsible for verifying
    V34 eligibility via `_v34_eligible()` before invoking the helper.
    ...
    def test_output_shapes_match_inputs(self, monkeypatch):
        ...
        # Need (O, L) from a V34 forward; use the same wrapper
        # `_make_mfa_custom` would have produced.
        from mlx_mfa._ext import v6_nax_forward
        O, L = v6_nax_forward(q, k, v, False, True)  # force_v34=True
        dO = mx.ones_like(O)
    ...
    def test_split_path_is_default(self, monkeypatch):
        """Without `MFA_V34BWD_USE_FUSED=1`, helper takes the
        ...
        monkeypatch.setenv("MFA_ENABLE_V34_BACKWARD", "1")
        monkeypatch.delenv("MFA_V34BWD_USE_FUSED", raising=False)
    ...
    def test_fused_path_via_env(self, monkeypatch):
        """With `MFA_V34BWD_USE_FUSED=1`, helper takes the legacy
        ...
        dQ_f, dK_f, dV_f = _v34_backward_vjp(q, k, v, O, L, dO, scale)
        _AE(dQ_f, dK_f, dV_f); mx.synchronize()

        monkeypatch.delenv("MFA_V34BWD_USE_FUSED")
        dQ_s, dK_s, dV_s = _v34_backward_vjp(q, k, v, O, L, dO, scale)
