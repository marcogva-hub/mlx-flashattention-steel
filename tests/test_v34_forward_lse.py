"""Tests for V6NAX forward lse-write patch (BLK1 resolution per
...
def force_v6nax(monkeypatch):
    """Force V6NAX path for D=64 small-N shapes that would otherwise route
    ...
    pre-existing and uses a different convention (log2-domain) than V6NAX's
    natural-log convention introduced by this BLK1 patch.

    Backward consumers (future V6NAX backward sprint) will only route to
    V6NAX backward when V6NAX forward was used — so this fixture matches the
    actual production routing pattern.  Tests that target V6NAX lse on
    D=64 force V6NAX explicitly.
    """
...
# ---------------------------------------------------------------------------
def test_v6nax_forward_output_matches_sdpa_d128_fp16():
    """O output from V6NAX forward (post-lse-patch) matches MLX SDPA
    ...
    assert rmse < 1e-3, f"V6NAX forward O regressed: RMSE {rmse:.4e}"


def test_v6nax_forward_output_matches_sdpa_d64_fp16(force_v6nax):
    ...
    assert rmse < 1e-3, f"D=64 V6NAX forward O regressed: RMSE {rmse:.4e}"


# ---------------------------------------------------------------------------
# lse correctness vs mx.logsumexp reference
# ---------------------------------------------------------------------------
def test_v6nax_forward_lse_matches_reference_d128_fp16():
    """lse from V6NAX forward matches mx.logsumexp(QK^T * scale) reference
...
def test_v6nax_forward_lse_matches_reference_d64_fp16(force_v6nax):
    q, k, v = _make_qkv(1, 4, 512, 64, seed=43, dtype=mx.float16)
    scale = 1.0 / math.sqrt(64)
    _, lse = _ext.v6_nax_forward(q, k, v, False)
    _mat(lse)
...
def test_v6nax_forward_lse_matches_reference_d128_bf16():
...
# ---------------------------------------------------------------------------
def test_v6nax_forward_lse_shape_and_finiteness():
...
# ---------------------------------------------------------------------------
def test_v6nax_forward_lse_last_block_remainder():
    """qL not aligned to V6NAX_BQ: last block has remainder rows; lse for
    those rows must still be written correctly (not garbage from
    unwritten memory)."""
    # V6NAX_BQ for D=128 is 16 (see Primitive eval_gpu line 528).
