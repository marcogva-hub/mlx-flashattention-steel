...
def test_v6_nax_varlen_seed_target_gqa2_is_finite_and_deterministic(monkeypatch):
    """Lock the long packed geometry that previously exposed a test-side tile mismatch."""
    lengths = [1750] * 19 + [1768]
    assert sum(lengths) == 35018
    monkeypatch.setenv("MFA_V6_NAX_BQ", "32")
    monkeypatch.setenv("MFA_V6_NAX_BK", "32")
    monkeypatch.setenv("MFA_V6_NAX_WM", "2")
    ...
    cu = mx.array(_prefix(lengths), dtype=mx.int32)
    tiles = _tile_offsets(lengths, 32)
    call = lambda: _ext.v6_nax_varlen_forward(
        q, k, v, cu, cu, tiles, 1 / math.sqrt(128), False
    )[0]
    first, second = call(), call()
    mx.eval(first, second)
    assert not bool(mx.any(mx.isnan(first)))
    assert not bool(mx.any(mx.isinf(first)))
    assert not bool(mx.any(mx.isnan(second)))
    assert not bool(mx.any(mx.isinf(second)))
    assert float(mx.max(mx.abs(first.astype(mx.float32) - second.astype(mx.float32)))) == 0.0
