...
|---|---|---|---|---|
| Backward dense D=64 non-causal qL≥2048 | `mx.vjp(SDPA)` | `mx.vjp(SDPA)` | **SDPA-vjp (default)**; native V6NAX backward experimental/opt-in | **(A)** correctness only. **Speedup UNVERIFIED** — prior 2.00×/1.95×/1.72× + 2.55×/3.76× withdrawn as measurement artifacts (settled 2026-06-19 at kernel level; see API_MANUAL backward note). |
...
| Backward causal **D=64** qL≥2048 | `mx.vjp(SDPA)` | `mx.vjp(SDPA)` | **SDPA-vjp (default)**; native V6NAX backward experimental/opt-in | **(A)** correctness only. **Speedup UNVERIFIED** — the prior 4.88×/5.75× was a measurement artifact (dQ-only timing vs SDPA full backward); withdrawn (settled 2026-06-19 at kernel level; see API_MANUAL backward note). |
