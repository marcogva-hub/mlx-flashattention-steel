...
| Backward dense **D=128** non-causal qL≥2048 | `mx.vjp(SDPA)` | `mx.vjp(SDPA)` | **V6NAX NAX-direct split kernels** (opt-in `MFA_ENABLE_V6_BACKWARD=1`, post-Prompt 5b Section D) | **(A)** **0.54× — i.e. SLOWER than SDPA-vjp** (M5/macOS 26.6/MLX-0.31.2, measured round-5 2026-06-22; engagement-proven, fresh inputs). Opt-in only, correctly NOT default; AUTO D=128 backward stays SDPA-vjp. (Earlier "parity / no speedup" was stale.) |
...
| Backward causal **D=128** qL≥2048 | `mx.vjp(SDPA)` | `mx.vjp(SDPA)` | **V6NAX NAX-direct split kernels** (opt-in `MFA_ENABLE_V6_BACKWARD=1`, post-Prompt 5b Section D + Prompt 4 multi-gate fix) | **(A)** **0.57× — i.e. SLOWER than SDPA-vjp** (M5/macOS 26.6/MLX-0.31.2, measured round-5 2026-06-22; engagement-proven). Opt-in only, correctly NOT default; AUTO D=128 backward stays SDPA-vjp. (Earlier "parity coverage extension" was stale.) |
