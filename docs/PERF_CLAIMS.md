...
| `iii2_tq_paged_decode_step_default` | III-2 (2026-06; re-confirmed III-12b on 26.6) | TQ paged decode step (per-step gather/dequant + Apple SDPA) is **6.5-23x faster than the FUSED TQ attend kernel it replaced** — re-measured 26.6: ~6.5-13x @S=4K, ~22x @S=16K (`0.75 ms vs 16.8 ms`). NOT vs fp16 dense (it is ~1.4-3x SLOWER than fp16 dense; TQ's value is ~4-5x KV memory at cos ~0.96). Baseline = the fused TQ kernel, not fp16 (lesson #15). | env unset (opt-out `MFA_DISABLE_TQ_DECODE_SDPA=1`) | `TurboQuantPagedInferenceContext.step(q, k, v)` N_q=1, B=1 Hq=32 Hkv=8 D=128 tq3b; reproduce: `benchmarks/methodology/iii12b_tq_claim_26.6_run{1,2}.log` (script `tq_claim.py`) | REACHABLE (default; kernel-cache-verified) |
