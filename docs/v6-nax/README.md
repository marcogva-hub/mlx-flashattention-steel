...
The previous v2.31.0 documentation described V6NAX NAX-direct as the production
default for D=128 and D=64 N_kv > 8000. **In v2.32.0, V6NAX is no longer
auto-dispatched on canonical shapes** — Apple's NAX kernel matches V6NAX cross-
session, and routing to it stops unnecessary competition with upstream tuning.
V6NAX remains in the codebase as the dispatched path when:

- `MFA_DISABLE_SDPA_ROUTE=1` is set (recovers v2.31.0 dispatch)
- Sprint A.6 empirical carve-outs match (specific shape-corners where V6NAX/V6
  NAX beats SDPA NAX)
- D=256/512 (not covered by SDPA NAX) — V2 D-split dispatched, NOT V6NAX
...
# V6NAX-backward training carve-out: `_v6nax_backward_carveout()` in dispatch_policy
...
materialized).  V6NAX-backward env-var carve-out is in
`_v6nax_backward_carveout()`.  If a future Sprint A.6 surfaces empirically-
...
V6 NAX and V6NAX therefore exist as:
- A research/bench path callable explicitly (`bench/v6nax_bench.py`,
...
| Variant | When | Source |
|---|---|---|
| **`createV6NAXSource()`** (NAX-direct) | Default for D=128 (any shape), and D=64 with `N_kv > 8000` (e.g. LTX2-cross). Self-contained MSL emit (~17.7 KB inlined Apple helpers + ~400 LOC kernel body). Uses Apple's `NAXFrag::mma` directly with `metal::execution_simdgroup` (singular) — no MPP cooperative_tensor at `<N>`. | `csrc/mfa/v6_nax/NAAttentionKernel.cpp::createV6NAXSource` (~700 LOC, added in Sprint V6NAX) |
| **`loopForwardSingleTile()`** (legacy single-Otile, Apple-style via MPP) | Default for D=64 small-N (FlashVSR-dense style — V6NAX regresses ~39% on small symmetric self-attention). Single cS, kBlocks=1, always-bypass cP cooperative tensor, `mem_none` barriers. | `csrc/mfa/v6_nax/NAAttentionKernel.cpp::loopForwardSingleTile` (~270 LOC, added in Sprint 3.3) |
...
V6NAX constraints: `BQ % (WM * 16) == 0`, `BD % 16 == 0`, `TQ = BQ / (WM * 16) == 1`.
...
The v2.31.0 release performance table (V6NAX +33-40% wins on D=128) was
...
- Phase 0 cross-session A/B/A re-bench measured legacy V6 NAX 36-41%
  faster on D=128 than the v2.31.0 v6nax-aba.json data showed (same
  hardware, same code).
...
Numerical accuracy on canonical D=128 shapes: V6NAX RMSE FP32 vs SDPA
...
canonical shapes is Apple's NAX kernel directly — V6NAX numerics matter
...
| V6NAX | NAX-direct rewrite via `NAXFrag::mma` / `NAXTile` — bypasses MPP cooperative_tensor entirely. SDPA parity on D=128 (3 shapes) and D=64 N≥2048 (1 shape). SeedVR2-small beats SDPA at 0.89×. | [v6nax-results.md](v6nax-results.md), [v6nax-apple-reference-mapping.md](v6nax-apple-reference-mapping.md) |
...
3. **Causal forward** falls through to SDPA NAX on M5+ canonical shapes
   ...
   V6NAX still support causal for `MFA_DISABLE_SDPA_ROUTE=1` runs and
   carve-out shapes.
4. **The v2.31.0 V6NAX numbers are non-reproducible cross-session.** See
...
- Apple `steel_attention_nax.h` (V6NAX's primary reference) —
...
- Apple `defines.h` / `utils/integral_constant.h` / `kernels/utils.h::Limits` —
  helpers inlined into V6NAX's emitted MSL.
...
- V6NAX sprint docs:
  [`v6nax-apple-reference-mapping.md`](v6nax-apple-reference-mapping.md),
  [`v6nax-results.md`](v6nax-results.md),
  [`v6nax-aba.json`](v6nax-aba.json).
- Bench scripts — `bench/v6_*.py`, `bench/v6nax_*.py/sh`
