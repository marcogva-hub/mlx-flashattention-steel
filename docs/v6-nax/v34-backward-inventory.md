# V34 backward NAX-direct (Option β) — inventory

## Goal

Implement NAX-direct backward attention kernels (dQ + dK/dV) per
`docs/v6-nax/v34-backward-option-beta-design-hints.md`, transferring
the V34 forward B+C+E mechanism bundle (cross-SG sync elim + simd_shuffle_xor
...
| File | Role | Verified |
|---|---|:--:|
| `docs/v6-nax/v34-backward-option-beta-design-hints.md` | Canonical input (5 open Qs, B+C+E transfer plan) | ✓ |
| `csrc/mfa/v6_nax/NAAttentionKernel.cpp::createV34Source()` | V34 forward source (658 LOC, lines 2307-2964) — clone target for backward kernel structure | ✓ |
...
| `csrc/mfa_v6_nax_primitive.cpp::v6_nax_forward()` (line 696) | V34 forward Python-callable Primitive — returns `(O, lse)` pair | ✓ |
| `mlx_mfa/attention.py` custom_vjp registration | Python autograd hook point | not yet read |

## V34 forward kernel signature (current state, line 2759)
...
**Only 5 buffers — NO `L` (lse) buffer.** The V34 forward Primitive
...
not.** V34 forward as currently shipped does not compute lse into
device memory. This is the foundational blocker for V34 backward
dQ implementation.
...
|---|---|---|---|
| (a) Extend V34 forward to write lse | ~50 LOC patch to forward kernel + Primitive | Cleanest. Forward outputs match Apple SDPA-NAX contract. Backward depends on existing path. | Modifies production forward kernel — risk of forward regression. Must re-bench V34 forward post-patch. Was explicitly listed "out of scope" in original prompt §1 but is necessary infrastructure for backward. |
| (b) Add `compute_lse_from_v34` helper kernel | ~30 LOC new kernel + binding | Forward kernel untouched. New kernel scoped tightly. | +1 dispatch per backward call. Helper kernel needs to RECOMPUTE max + sum from device output — wasteful (O(N²) recomputation). |
| (c) Use STEEL forward → V34 backward hybrid path | ~0 kernel change | Smallest scope. Uses existing STEEL forward (which writes lse). | V34 backward depends on STEEL forward state layout — fragile cross-kernel coupling. Eliminates V34 forward perf gain when paired with V34 backward. |
...
anti-pattern A bonus finding). Adding lse-write to V34 forward is a
...
`L.store_rows(...)` or analog). Re-bench V34 forward post-patch under
canonical methodology to confirm no regression.

This is **BLK1** — top-priority resolution before Phase 1 Section B
can begin.

## Shape catalog (V34 backward eligibility, per design hints scope)
...
| V34 backward ratio vs STEEL backward | < 1.0 (V34 faster) across eligible regime |
| Realistic target | ratio ∈ [0.4, 0.85] (15-60% speedup) |
| Three-axis tests | All axes covered, ≥ 15 new tests |
| Auto-routing | `flash_attention()` VJP transparently uses V34 on M5+ eligible |
| Escape hatch | `MFA_DISABLE_V34_BACKWARD=1` falls back STEEL |
...
- Block-sparse backward (causal mask interaction with V34 backward)
- Causal backward (deferred per DC3 in design hints)
- V34 forward EXEC_SG shape-aware heuristic patch (independent ~1h
