...
**Branch:** `experiment/v34-forward-max` @ `8a389c3`
**Hardware:** M5 Max (`applegpu_g17s`), iStat performance fan profile active
**Bench:** `bench/v34_aba_wrapper.sh` — 5 production shapes × L/V/L (legacy → V34 → legacy) A/B/A
...
before merging. Sprint 4 reported a sign-flip — turning a "V34 −39%
regression on FlashVSR-dense" into a "V34 +20% win" via a tile-config
...
| Shape | R1 legacy ms | R2 V34 ms | R3 legacy ms | R1↔R3 drift | SDPA stable? | V34 vs legacy avg | V34/SDPA |
|---|---:|---:|---:|---:|:---:|---:|---:|
| FlashVSR-dense | 0.93 | 0.95 | 1.74 | **86.7% ❌** | 0.89→0.91→1.08 (drifted) | 0.713× (artifact) | 1.044× |
| LTX2-cross | 1.65 | 1.30 | 1.61 | 2.5% ✅ | 1.34/1.33/1.33 ✅ | **0.797× = +20% V34** | **0.973×** |
| SeedVR2-small | 162.13 | 184.73 | 173.37 | 6.9% ✅ | 184.92/197.35/184.68 ✅ | 1.101× = **−10% V34** | 0.936× |
| CogVideoX | 2335.85 | 2162.00 | 2352.43 | 0.7% ✅ | 2194/2187/2183 ✅ | **0.922× = +8% V34** | **0.989×** |
| SeedVR2-large | 4073.61 | 3878.19 | 3891.03 | 4.5% ✅ | 3831/3797/3762 ✅ | 0.974× = **+3% V34** | 1.021× |

### Acceptance verdict (per shape)

| Shape | Sprint 4 / v2.31.0 claim | Phase 0 finding | Verdict |
|---|---|---|---|
| FlashVSR-dense | Sprint 4 +20% V34 default | Drift 86.7% — measurement invalid. R1↔R2 cold comparison: V34 −2% | **NOT REPLICATED.** Sprint 4 +20% claim does not survive cross-session. |
| LTX2-cross | Sprint 4 +14% V34 default | Drift 2.5% — clean. V34 +20%, V34/SDPA 0.973× | **REPLICATED ✅** (even better than Sprint 4 claim). |
| SeedVR2-small | v2.31.0 +36% V34 default | Drift 6.9% — clean. V34 −10% | **NOT REPLICATED ❌** — V34 is now slower than legacy. v2.31.0 +36% was thermally-inflated comparison. |
| CogVideoX | v2.31.0 +34% V34 default | Drift 0.7% — clean. V34 +8% | **PARTIALLY REPLICATED.** Real V34 win, much smaller than v2.31.0 marketed. |
| SeedVR2-large | v2.31.0 +40% V34 default | Drift 4.5% — clean. V34 +3% | **MARGINALLY REPLICATED.** Real V34 win, much smaller than v2.31.0 marketed. |

## Cross-session legacy drift

The most striking finding is not V34's behavior but **how much faster
legacy is today than in the v2.31.0 v34-aba.json data** (which was the
...
This means **v2.31.0's headline "V34 wins +33-40% on D=128"** was the
ratio of (v2.31.0 V34) over (v2.31.0 thermally-penalized legacy). Today's
ratio of (Sprint 5 V34) over (today's faster legacy) is much smaller —
...
| 4 — D=64 BK=64 → BK=32 default | **Robust improvement** — closes the real −39% regression on FlashVSR-dense (V34 BK=64 was 1.55ms → BK=32 is 0.95ms). Ships. |
| 4 — V34 always-on dispatch for D=64 | **Sprint 4 +20% claim does NOT replicate.** V34 is at parity (FlashVSR-dense) or wins big (LTX2-cross). Per Phase 0 spec, **revert** to v2.31.0 behavior: V34 default for D=64 N_kv > 8000 only; legacy default for D=64 small-N. |
...
Phase 0 surfaced a v2.31.0 dispatch issue: V34 is currently the default
for **all D=128 shapes**, but cross-session data shows V34 regresses 10%
on SeedVR2-small while winning +8% on CogVideoX and +3% on SeedVR2-large.
The v2.31.0 release table that motivated "V34 universal D=128 default"
...
1. **Document only.** Note in CHANGELOG that SeedVR2-small specifically
   regresses ~10% under V34 vs legacy on M5 Max in current measurements;
   recommend `MFA_V6_USE_V34=0` override for that shape. No code change
   beyond Sprint 4 dispatch revert.

2. **Code carve-out.** Add a per-shape D=128 dispatch refinement: V34
...
```cpp
// V34 dispatch logic — same as v2.31.0
if (D == 128) { use_v34 = true; }
else if (D == 64 && N_kv > 8000) { use_v34 = true; }
else { use_v34 = false; }

// V34 D=64 tile defaults — Sprint 4 BK fix, KEPT:
v34_BQ = (head_dim == 64) ? 32 : 64;
v34_BK = (head_dim == 64) ? 32 : 32;  // ← was 64, now 32 (Sprint 4)
v34_WM = (head_dim == 64) ? 2 : 4;
```
...
- **R2 SDPA on SeedVR2-small drifted +7%** (185 → 197) — its V34
  measurement is therefore mildly thermally penalized. Adjusted for
  thermal trend (V34 ≈ 184.73 × 184/197 ≈ 173ms), V34 is still slightly
  slower than legacy avg 167.75ms (~3-4%). The "V34 loses" finding holds
  but margin shrinks from 10% to 3-4%.
- **Single session.** Phase 0 used one bench session. Reproducibility
  across multiple sessions over hours/days would strengthen any "V34
...
- ✅ **Revert** Sprint 4's V34-always-on D=64 dispatch change to v2.31.0 behavior (V34 only for D=64 N_kv > 8000).
