...
Behaviour and defaults reflect v2.32.0 (post-V6NAX Forward-Max sprint suite).
...
## V6NAX (NAX-direct) flags — added in v2.31.0–v2.32.0

V6NAX is the Apple `steel_attention_nax.h`-style NAX-direct kernel
(`createV6NAXSource`), bypassing MPP cooperative_tensor entirely. Its
dispatch is shape-aware:

- D=128 (any shape) → V6NAX default
- D=64 (any N) → V6NAX default since v2.32.0 (Sprint 4 fix, was D=64 N≥8000 only in v2.31.0)
- D=256+ → not yet ported, falls back to legacy
- Causal forward → V6NAX supports it since v2.32.0 (Sprint 1)

| Var | Default | Effect |
|---|---|---|
| `MFA_V6_USE_NAX` | auto (per dispatch policy) | `1` forces V6NAX for any V6NAX-eligible shape; `0` forces legacy fallback. Useful for A/B comparison and bench wrappers. |
| `MFA_V6_NAX_BQ` | per-D auto (`D=64 → 32`, `D=128 → 64`) | V6NAX parallelization rows per Q-tile. Constraint: `BQ % (WM × 16) == 0`. |
| `MFA_V6_NAX_BK` | per-D auto (`D=64 → 32` since v2.32.0; `D=128 → 32`) | V6NAX traversal columns per K-tile. v2.32.0 changed D=64 default from 64 → 32 after Sprint 4 sweep showed BK=32 wins +14–20%. |
| `MFA_V6_NAX_WM` | per-D auto (`D=64 → 2`, `D=128 → 4`) | V6NAX simdgroups per threadgroup. |
<!-- MFA_V6_NAX_DISABLE_ALIGN removed (campaign 2026-06 Track 0): documented but never
...
For V6NAX-specific tuning, the per-D defaults committed in v2.32.0 were
...
`MFA_V6_NAX_BQ/BK/WM` unless you're specifically reproducing a sweep
result.
