# V6NAX backward Option γ — fused dK+dV with TGP cross-SG reduction (design)

Design analysis for the next-attempt sprint at V6NAX backward perf parity
vs SDPA-vjp.  Documented after v2.37.0 ship as Option β reached its
architectural floor at 2.4× SDPA-vjp.

## Motivation

V6NAX backward two-kernel split (Phase 2.O2 / v2.37.0) trades 2×
...
Estimated savings: 2-12ms (7-40% speedup).  Final V6NAX backward:
...
- Write `createV6NAXBackwardDKDVFusedSource()` ~700 LOC
...
- Add MFA_V6BWD_USE_FUSED_GAMMA=1 env var
...
- `docs/v6-nax/v6nax-backward-status.md` — full sprint timeline + Option β floor
- `docs/v6-nax/v6nax-backward-decisions.md` — DC0-DC13 design rationale
