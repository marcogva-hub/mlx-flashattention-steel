...
- [DELEGATED→agent aff9c7b] DOC-2..10, 12..15: README v2.39.1/v2.50/sparse/conv staleness, ENV_VARS V6NAX rows + missing opt-outs, HOOK_TELEMETRY, TRAINING_QUICKSTART, INVENTORY/INDEX headers, PERF_CLAIMS header, CLAUDE.md status, known-debt KD-7.
...
- [FIXED] F9 MEDIUM: unit-scale retrofit in all 6 V6NAX files + std-8 adversarial-finiteness test each. NO corruption discovered. Recalibrations (commented): kv dk_bound 1e-4→5e-4 (floor 5.7e-5, old margin 1.8x); dq bf16_bound default 1e-4→1e-3. v50_v6nax_causal: 0.1-defending comment removed — N(0,1) does NOT overflow fp16 softmax on M5 (measured floors dQ/dK 2.9e-3, dV 2.0e-3 vs bound 1e-2).
...
- [FIXED] F15 MEDIUM: ghost-env arm deleted with comment (grep-verified read nowhere in mlx_mfa/csrc); MFA_ENABLE_V6_BACKWARD (already set) is the real knob; opt_in kept as report annotation only.
...
- [FIXED+locked — force_kernel threaded; backend='mfa' runs MFA kernel (fp16 floor 0.001, not bit-SDPA); tests/test_phase3_iii4_dispatch_guards.py] D8 MEDIUM: backend="mfa" silently routed to SDPA forward on V6NAX-eligible cells (II-8 fusion branch) → forced-backend measurement corruption class; thread backend/force flag.
- [FIXED — v.shape[3]==D and kv_len==qL gates added to carve-out] D9 MEDIUM: V6NAX carve-out misses v_dim_mismatch guard + ignores kv_len (cross-attention can enter V6NAX backward) → add guards.
...
- [FIXED+locked — CONFIRMED real (dV RMSE 0.506 at bt=32); bt>=64 eligibility gate; tests/test_phase3_iii4_d16_sparse_bwd.py] D16 MEDIUM(conf HIGH): KD-1 OR-downsample may contaminate V6NAX sparse backward grads (env-gated path) → probe numerically vs SDPA-vjp; restrict or document.
- [FIXED] D17 LOW: stale docs (attn_bias "always falls back", make_causal_block_mask summary, _M5_NAX_THRESHOLDS comment, _v6nax_eligible qL=4096 docstring).
...
- [FINDINGS] dtype/§AA.5.x multi-gate sweep (agent a6258510):
  ...
  - Class C (sparse block-mask geometry) — [CLEAN] fully covered; no geometry-drift consumer beyond the 3 pass-1 D4 sites (V2-V5 exclude masks at eligibility; parameterized sparse threads explicit block_tile; V6NAX NAX arch-independent).
...
- [FINDINGS] Numerical edge + backward correctness (agent a2a5b8f, 13 empirical probes):
  ...
  - [CLEAN, measured]: NaN-input PROPAGATES on every fwd path (Rule 8 clean, no silent clamp); fp16 std12 no Inf (max|out|≤54); odd dims fall back to SDPA; zero-length varlen segment clean. EVERY differentiable path correct at unit AND std-8: V6NAX bwd D64 causal+noncausal (fp16/bf16; std8-bf16 "blowup" PROVEN bf16-precision via fp32-GT triangulation), alibi/window/softcap+window (D2 holds)/softcap, sparse bwd ×3, GQA/MQA bit-exact + H_kv shape, rope/packed-QKV/packed-KV bit-exact, GNA forward-only grad raises loudly.
...
- [CLEAN, live] V6NAX backward (env-gated, D=64 causal, M5): dQ/dK/dV rel 7e-4 vs SDPA-vjp, no NaN.
