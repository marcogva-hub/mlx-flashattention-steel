...
| `MFA_FORCE_NATIVE_BWD` disposition (Archaeology A, 2026-06-17) | **REMOVE-ELIGIBLE — deprecation cycle COMPLETE** (queue entry ACCURATE, not stale). Forces correct-but-dominated STEEL backward (measured: V34 wins D=64, SDPA-vjp wins D=128 — sprint-C Track 2; robust to 26.6); inert on default API (forward→SDPA); deprecation announced v2.50.0 "target removal v2.51+", now overdue at v2.55.0. Thin escape-hatch value only (`backend="mfa"` research/A-B). Removal = env-var knob, keep STEEL kernel (keep-all-paths). See `docs/v50/campaign-2026-06/queue-force-native-bwd-archaeology.md`. **Marco-gated — no action.** | removal: deprecation cycle done (no build) |
| V3/V4/V5 kernels disposition (Archaeology B, 2026-06-17) | **SPLIT.** **V4/V5 = CLEAN-KEEP** (opt-in via `enable_v4`/`enable_v5`, never auto, correct on M5 — V5 III-9 OOB-V clamp in + smoke-verified; queue framing accurate). **V3 = GAP + STALE-VERDICT** — V3 is **auto-routed** (causal, N≥4096/2048, B·H≥4; `mfa_attention.cpp:808-818`), NOT opt-in, and was measured to **WIN** vs V2 (1.015×, M1-Max-2026-03), so the "opt-in/regress" framing is doubly stale. Correct + OOB-safe on M5 (smem-staged V; window supported). But auto-fires for real users (windowed-causal-large-N default-reachable on M5) on an M1-2026-03 verdict never re-validated on M5/26.6 (tradeoff already flipped once on M1). See `docs/v50/campaign-2026-06/queue-v3v4v5-archaeology.md`. **Marco-gated next steps: (1) correct V3 framing; (2) Pattern #6 re-bench V3-vs-V2 on M5/26.6. No action.** | re-bench (no build) |
