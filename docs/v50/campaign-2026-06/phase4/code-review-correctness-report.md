...
**one HIGH latent** int32-overflow in the legacy/MPP V6 NAX attention path (V34-off + multi-GB
...
| **A3-1** | mlx-mfa | **HIGH** | CONFIRMED | legacy/MPP V6 NAX device offsets `tgid.z*batch_stride` etc. are 32-bit → wrap >2³² (silent wrong-mem); latent (V34-off + multi-GB / >4M varlen tokens) | **FIXED (Wave M1)** |
...
by construction, matching the V34 path's existing 64-bit arithmetic + the conv path's <2³¹ guard).
