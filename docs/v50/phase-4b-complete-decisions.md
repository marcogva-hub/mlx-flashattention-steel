# v2.50 Phase 4b-complete — V6NAX backward causal — PARTIAL
...
that `compile_v6nax_backward_pipeline` (Sprint v2.40.x consolidation
helper) **hardcoded `isCausal=false`** at line 852.  This meant:
- All 5 V6NAX backward source generators were compiled with
  `descriptor.isCausal == false`
- All `#define V6NAXBWD*_CAUSAL` macros evaluated to 0
- All `#if V6NAXBWD*_CAUSAL { mask block }` were COMPILED OUT
...
- `compile_v6nax_backward_pipeline` signature: added `bool isCausal = false` parameter
- 5 `V6NAXBwd*Key` structs: added `bool causal` field + hash
- 5 `MFAV6NAXBwd*` Primitive classes: added `causal_` member + ctor parameter
- 5 raw helpers (`v6_nax_backward_query`, etc.): added `bool causal` parameter
- 5 nanobind bindings: added `nb::arg("causal") = false`
- Python `_v6nax_backward_vjp(... , causal=False)`: pass causal through
...
The mask blocks are structurally identical to the V6NAX forward causal
mask (Prompt 2 Phase 4a) which is empirically correct.  But empirical
validation in this session showed:

| Metric | Result | Status |
|---|---|---|
| dV magnitude (V6NAX causal vs SDPA causal) | 10× SMALLER | structural under-counting |
| dV RMSE | 2.7e-3 (vs 1e-3 bound) | 2.7× over |
| dV correlation with SDPA | 0.73 | not perfect alignment |
| dV ratio (V6NAX/SDPA) | median 0.039 across heads | consistent scaling factor |
...
**Production decision**: `_v6nax_eligible(causal=True)` returns False
and `_v6nax_backward_carveout(causal=True)` returns False — production
...
## DC1 — The compile_v6nax_backward_pipeline isCausal bug
...
Source generators (e.g., `createV6NAXBackwardQuerySource()`) read
...
V6NAXBWD_CAUSAL 0`, and the `#if V6NAXBWD_CAUSAL` mask block was always
...
`MFA_V6BWDF_DUMP_SOURCE=1` and grep'd `V6NAXBWDF_CAUSAL` — found it
was 0 despite the test passing `causal=True` through Python.

**Fix**: Threaded `isCausal` through:
1. `compile_v6nax_backward_pipeline` accepts `bool isCausal` parameter
2. Each `V6NAXBwd*Key` cache key includes `bool causal` field
3. Each `MFAV6NAXBwd*` Primitive stores `bool causal_` and passes it
4. Each raw helper (`v6_nax_backward_query`, etc.) accepts `bool causal`
5. Each binding exposes `nb::arg("causal") = false`
6. Python `_v6nax_backward_vjp` accepts `causal` parameter and threads
   to each binding call
7. Python `_make_mfa_custom._backward` passes `causal` to `_v6nax_backward_vjp`
...
dV residual:
- Verified mask block is compiled in (V6NAXBWDF_CAUSAL = 1 in source dump)
- Verified mask predicate `(r < c) ? neg_inf : fg[loc]` is structurally
  identical to V6NAX forward's (which works perfectly)
...
- Correlation analysis showed V6NAX dV vs SDPA dV correlation 0.73, ratio
...
delivers:
1. **Critical latent bug fixed**: the `compile_v6nax_backward_pipeline`
   hardcoded `isCausal=false` would have continued to silently
   no-op any future causal kernel work
2. **dQ kernel now works**: V6NAX backward dQ for causal is now ready
...
| `mlx_mfa/attention.py` | +15 | `_v6nax_backward_vjp` accepts/threads causal; eligibility gate retained on causal |
| `mlx_mfa/dispatch_policy.py` | +5 | `_v6nax_backward_carveout` retains `not causal` constraint |
| `docs/v50/phase-4b-complete-decisions.md` | +200 (new) | this doc |

## Recommended next steps (focused future session)

1. Bench V6NAX backward causal at qL=64 with eligibility gate bypassed
   (single Q-block; eliminates qb-loop boundary effects from the
   investigation)
2. Add a debug-write to the dV kernel inside `#if V6NAXBWDF_CAUSAL`
...
4. Once residual resolved, lift eligibility gates (one-line revert in
   `_v6nax_eligible` and `_v6nax_backward_carveout`)
