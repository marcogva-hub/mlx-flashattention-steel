...
The Prompt 3 "dV K-parallel residual" (V6NAX backward causal dV
...
1. **Line 176** (`generate_v6_source`):
   ```cpp
   if (use_v6nax && (isCausal || !single_otile)) use_v6nax = false;
   ```
2. **Line 625** (eval_gpu pre-dispatch decision):
   ```cpp
   if (use_v6nax && (params_.causal || !so_for_v6nax)) use_v6nax = false;
   ```

Both gates silently routed `causal=True` forward to STEEL legacy
(`createSource()`'s `loopForward` template) instead of `createV6NAXSource()`.
STEEL legacy emits lse in **log2 domain** (S_log2 + log2(sum_score));
V6NAX backward kernels expect lse in **natural-log domain** (per the
Phase 4b dQ kernel comment at `NAAttentionKernel.cpp:4007`).

V6NAX backward consumed wrong-domain lse → all gradients wrong → dV at
~10× under-counting magnitude.

**Empirical proof — diagnostic with Q=K=V=dO=1 (qL=64, D=64)**:

Pre-fix lse output for V6NAX forward causal:
...
Post-fix V6NAX backward causal dV:
- c=0: V6NAX = 4.7422, SDPA-ref = 4.7422, ratio = 1.000 ✓
- c=63: V6NAX = 0.0156, SDPA-ref = 0.0156, ratio = 1.000 ✓
...
Both gates were leftover from pre-Phase-4a state when V6NAX forward
...
### `mlx_mfa/attention.py::_v6nax_eligible`
```diff
- if causal:
-     return False  # Phase 4b-complete deferred (K-parallel kernels)
```

### `mlx_mfa/dispatch_policy.py::_v6nax_backward_carveout`
```diff
  if (
      head_dim == 64
      and seq_len >= 2048
...
Prompt 3 added `#if V6NAXBWD*_CAUSAL` mask blocks to all 4 K-parallel
kernels (dV split, dK split, dKV legacy fused, dKdV fused).  These
are now **structurally correct AND necessary** — the V6NAX backward
...
(natural-log lse), the V6NAX backward kernels would still produce
...
just enables them to consume the correct natural-log lse from V6NAX
forward causal.
...
2. Did not trace the ROUTING: I assumed force_v6nax=True flowed through
   to the V6NAX kernel, but the dispatch gates silently overrode it
...
- Don't assume `force_v6nax=True` end-to-end without tracing the
  dispatch chain
- A "force_v6nax=True" parameter should HONESTLY override; the current
  gates create silent fallback behavior that violates the contract

## Three-axis validation

### Axis 1 — Output correctness (V6NAX backward causal vs SDPA-vjp)
...
now engages V6NAX backward causal at D=64 qL≥2048 fp16/bf16.  Test
`test_flash_attention_causal_engages_v6nax` verifies V6NAX dispatch and
gradient correctness.

### Axis 3 — Edges preserved

- V6NAX backward non-causal: unchanged (RMSE 6e-6 — same as pre-fix)
- V6NAX forward causal: now emits natural-log lse correctly (was log2)
- V6NAX forward non-causal: unchanged (already correct)
- STEEL legacy backward (M1/M3 fallback): unaffected (only routing
  path changed for M5+ V6NAX-eligible causal)
...
| Phase | Skill | Status |
|---|---|---|
| 4b.dv.0 §AA.5 premise check | `/mlx-mfa-apple-primitives-coverage` | done — CONFIRMATION (no Apple primitive provides V6NAX-direct backward) |
...
| 4b.dv.2 dispatch gate investigation | manual (grep `use_v6nax` + `isCausal` in source) | done — found 2 dispatch gates at lines 176, 625 |
| 4b.dv.3 fix + validation | Empirical diagnostic harness (`/tmp/dv_diagnostic.py`) | done — V6NAX dV ratio 1.000 vs SDPA ref |
...
| `mlx_mfa/attention.py` | +0 (replaced gating comment) | Remove `if causal: return False` from `_v6nax_eligible` |
| `mlx_mfa/dispatch_policy.py` | +0 (replaced gating comment) | Remove `not causal` from `_v6nax_backward_carveout` |
| `tests/test_v6nax_helpers.py` | +5 (rename + update) | `test_d64_fp16_causal_returns_false` → `_returns_true` |
| `tests/test_v39_fused_dkdv.py` | +6 (rename + update) | `test_v6nax_eligible_causal_false` → `_true` |
| `tests/test_v50_v6nax_causal.py` | +20 (rename + behavior swap) | `test_sprint4_v6nax_eligibility_causal_returns_false` → `test_v6nax_eligibility_causal_returns_true`; `test_sprint4_flash_attention_causal_uses_sdpa_vjp` → `test_flash_attention_causal_engages_v6nax` (now asserts V6NAX engagement + correct gradients with scaled inputs) |
| `docs/v50/phase-4b-complete-dv-residual-decisions.md` | +200 (new) | this doc |

## Net effect on users

- `mx.grad(flash_attention(q, k, v, causal=True))` with
  `MFA_ENABLE_V6_BACKWARD=1`, D=64, qL≥2048, fp16/bf16 — now engages
  V6NAX NAX-direct backward causal kernels on M5+, producing correct
  gradients (RMSE within fp16 ULP).
- For non-V6NAX-eligible callers (D=128, smaller qL, fp32, etc.): SDPA-vjp
  fallback unchanged.
- For non-causal callers: unchanged (V6NAX backward non-causal already
  worked correctly).
