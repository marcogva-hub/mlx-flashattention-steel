...
**Phase 4a status**: SHIPPED (V34 forward causal)
**Phase 4b status**: SHIPPED partial (dQ kernel causal mask only)

## TL;DR

V34 backward causal end-to-end requires 4 additional K-parallel
...
When `_v34_eligible(causal=True)` lifts to allow V34 backward causal,
the V34 backward dispatcher (`_v34_backward_vjp`) calls 3 kernels:
...
```cpp
ss << "#define V34BWD<X>_CAUSAL " << (isCausal ? 1 : 0) << "\n";
```
...
#if V34BWD<X>_CAUSAL
{
  ...
  const int base_row = qb * V34BWD<X>_BQ + params.qL_off;
  ...
  const int base_col = int(tid.x) * V34BWD<X>_BK
                       + simd_group_id * (V34BWD<X>_BK / V34BWD<X>_WM);
  for (iq, ik, ii, jj):
...
Computing:
```cpp
const int K_max = (int(tid.x) + 1) * V34BWD<X>_BK
                  + simd_group_id * (V34BWD<X>_BK / V34BWD<X>_WM);
int qb_min = metal::max(0, (K_max - 1 - params.qL_off) / V34BWD<X>_BQ);
```
...
- `base_col = int(tid.x) * V34BWDKV_BK`
...
- `base_col = int(tid.x) * V34BWDV_BK + simd_group_id * (V34BWDV_BK / V34BWDV_WM)`
...
- `base_col = int(tid.x) * V34BWDK_BK + simd_group_id * (V34BWDK_BK / V34BWDK_WM)`

### Fused dKdV (newer, replaces legacy fused dKV when MFA_V34BWD_USE_FUSED=1 NOT set)
...
| Test legacy fused dKV vs SDPA-vjp causal via `MFA_V34BWD_USE_FUSED=1` | 15 min |
...
| Lift `_v34_eligible` and `_v34_backward_carveout` causal gates | 5 min |
...
- **Phase 4a SHIPPED at Sprint 4 branch** (will merge to master with
  this STATUS doc): V34 forward causal kernel + non-causal regression
  verified.
- **Phase 4b dQ SHIPPED**: dQ kernel causal mask block (compiled
  only when `V34BWD_CAUSAL=1`, which fires only when the eligibility
  gate is lifted — currently it isn't).
- **Test count post-merge**: 1140 passing (1129 baseline + 11 Sprint 4).
- **Production safety**: `_v34_eligible(causal=True)` returns False;
  `_v34_backward_carveout(causal=True)` returns False.  Callers using
  `flash_attention(causal=True)` with `MFA_ENABLE_V34_BACKWARD=1`
...
3. Implement the 4 mask blocks one kernel at a time, testing each
   independently before proceeding (use `MFA_V34BWD_USE_FUSED=0/1` +
   targeted `_v34_eligible(causal=True)` override for testing).
...
(`feat/v50-sprint4-v34-causal`) ships Phase 4a + Phase 4b dQ
infrastructure ONLY.

## Master state post-Sprint-4 (Phase 4a + dQ infrastructure)

- `master`: ~`<Sprint 4 merge commit>`
- Sprint 4 Phase 4a complete: V34 forward causal extension.
