...
**Mandate**: lift `_v6nax_backward_carveout` D=128 hard-gate; broaden V6NAX
...
or consume V6NAX backward inputs for D=128 BEFORE the broadening fix.
...
|---|---|---|---|---|
| G1 | `_v6nax_backward_carveout` (flash_attention()-level carve-out) | `mlx_mfa/dispatch_policy.py:373-380` | **BLOCKING**: `head_dim == 64` | **LIFT** → `head_dim in (64, 128)` |
| G2 | `_v6nax_eligible` (closure-level second-line check) | `mlx_mfa/attention.py:3743` | PERMISSIVE: `head_dim in (64, 128)` | Verified; no change |
| G3 | `_v6nax_backward_vjp` routing | `mlx_mfa/attention.py:3827-3859` | PERMISSIVE: `head_dim not in (64, 128)` → ValueError for `fused`; AUTO routes D=128 → `split` per Sprint B outcome γ | Verified; no change |
| G4 | `MFAV6Forward::eval_gpu` causal-routing gate | `csrc/mfa_v6_nax_primitive.cpp:625` (post-Prompt 4 fix) | PERMISSIVE: causal+D=128 routes to V6NAX forward (natural-log lse) | Verified; no change |
...
| G6 | `compile_v6nax_backward_pipeline` cache keys | C++ Primitive cache | PERMISSIVE: D=128 cache keys constructed | Verified; no change |
...
- **G4 verification**: Prompt 4 Section B already lifted the causal routing
  gate to enable V6NAX forward causal (natural-log lse) for D ∈ {64, 128}.
  D=128 causal forward currently produces natural-log lse; V6NAX backward
...
- **G5 verification**: Sprint B v2.40.0-internal Phase C.1.b added
  ...
  D=128 opens via opt-in `MFA_V6_BWD_KERNEL=fused`.

## LSE consistency check (per `docs/methodology/kernel-debugging.md` §4)

V6NAX forward D=128 emits natural-log lse (post-Prompt 4 multi-gate fix).
V6NAX backward D=128 split kernels expect natural-log lse. **Convention
match confirmed**.
