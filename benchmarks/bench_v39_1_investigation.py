...
mx.grad(flash_attention(..., backend="auto")) with MFA_ENABLE_V34_BACKWARD=1.
...
def _set_env(arm, **overrides):
    ...
    overrides (MFA_V34BWDF_BQ, _BK, _WM, MFA_V6_MAX_THREADS)."""
    keys = (
        "MFA_V34_BWD_KERNEL", "MFA_DISABLE_V34_BACKWARD",
        "MFA_ENABLE_V34_BACKWARD",
        "MFA_V34BWDF_BQ", "MFA_V34BWDF_BK", "MFA_V34BWDF_WM",
        "MFA_V6_MAX_THREADS",
    )
    for k in keys:
...
def run_h3_occupancy_sweep(out):
    ...
    for name, bq, wm in configs:
        _set_env("fused", MFA_V34BWDF_BQ=bq, MFA_V34BWDF_WM=wm)
        try:
...
def run_h1_bk_sweep(out):
    ...
    configs = [
        ("baseline_BK32", {"MFA_V34BWDF_BK": 32}),
        ("BK16", {"MFA_V34BWDF_BK": 16}),
        ("BK32_maxthr128", {"MFA_V34BWDF_BK": 32, "MFA_V6_MAX_THREADS": 128}),
    ]
    qL, D = 4096, 64
    rows = []
