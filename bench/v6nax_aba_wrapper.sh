#!/usr/bin/env bash
# V6NAX vs legacy A/B/A cross-session bench wrapper.
# 5 production shapes × {legacy, v6nax, legacy_validation} rounds.
# 90s inter-round cooldown, 30s inter-shape cooldown for thermal stability.
set -e
cd /Users/marcomarcelino/code/mlx-mfa-v2

OUT=outputs/v6nax-aba.json
LOG=outputs/v6nax_aba.log
SHAPES=("FlashVSR-dense" "LTX2-cross" "SeedVR2-small" "CogVideoX" "SeedVR2-large")

mkdir -p outputs
rm -f "$OUT"

echo "=== V6NAX A/B/A bench start $(date) ===" | tee "$LOG"
...
for shape in "${SHAPES[@]}"; do
  ...
  # Round 1: legacy
  .venv/bin/python bench/v6nax_bench.py --shape "$shape" --mode legacy --runs 3 \
                   --include-sdpa --output "$OUT" 2>&1 | tee -a "$LOG"
  sleep 60
  # Round 2: v6nax
  .venv/bin/python bench/v6nax_bench.py --shape "$shape" --mode v6nax --runs 3 \
                   --include-sdpa --output "$OUT" 2>&1 | tee -a "$LOG"
  sleep 60
...
echo "=== V6NAX A/B/A bench done $(date) ===" | tee -a "$LOG"
